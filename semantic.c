...
void set_literal_address(A_NODE *node) {
	...
	for (i=1;i<=literal_no; i++)
		literal_table[i].addr+=node->value; //아직 addr이 이해가 잘 안가는걸?
...
int put_literal(A_LITERAL lit, int ll) {
	...
	if (literal_no >= LIT_MAX)
		...
		literal_no++; //이러면 literal_no 가 99인 상황에 100을 들어가는거 아니야?
...
A_TYPE *sem_expression(A_NODE *node) {
	...
	switch(node->name) {
		case N_EXP_IDENT :  //ident노드 일때는 심볼 테이블이 있어야함
			id=node->clink;
			switch (id->kind) { 
				case ID_VAR:
				case ID_PARM: 
		...
		case N_EXP_STRING_LITERAL : 
			...
			node->clink=put_literal(lit,node->line); //index
			result=string_type;
			break;
		case N_EXP_ARRAY :
		...
		case N_EXP_SUB :
			...
			if (isArithmeticType(t1) && isArithmeticType(t2))
				...
				result=t1; 
			else    //왼쪽이 정수고 오른쪽이 포인터인 그런경우는 에러야? 그리고 그때는 result가 t1이 맞아?
				semantic_error(24,node->line,""); 
			break;
		case N_EXP_LSS : 
		case N_EXP_GTR : 
		...
		case N_EXP_GEQ :
			...
			t2=sem_expression(node->rlink);
			if (isArithmeticType(t1) && isArithmeticType(t2))
				t=convertUsualBinaryConversion(node); //t? return? 이게 리턴을 변수에 받는게 의미 있는 코드인가?
			else if (!isCompatiblePointerType(t1,t2))
		...
		case N_EXP_EQL : 
			...
			t2=sem_expression(node->rlink);
			if (isArithmeticType(t1) && isArithmeticType(t2))
				t=convertUsualBinaryConversion(node); //위의 GEQ와 동일
			else if (!isCompatiblePointerType(t1,t2) &&
		...
		case N_EXP_ASSIGN :
			...
			if(t->kind==T_FUNC){
				...
				t->size=4; 
				node->rlink=makeNode(N_EXP_AMP,NIL,node->rlink,NIL); 
				node->rlink->type=t;
			}
			// debug();
			// check modifiable lvalue
...
{
	//보이드가 있는게 맞는데, 어차피 void변수는 나오면 포인터 빼고 다 없앨거고 걔는 결국 포인터라 의미 없음
...
int sem_statement(A_NODE *node, int addr, A_TYPE *ret, BOOLEAN sw, BOOLEAN brk, BOOLEAN cnt)
{
	...
	switch(node->name) {
		...
		case N_STMT_SWITCH: 
			t=sem_expression(node->llink); 
			if (!isIntegralType(t))	//책에는 scalar로 되어있는데? integral이 맞는듯
				semantic_error(50,node->line,"");
...
int sem_declaration(A_ID *id,int addr)
{
	...
	switch (id->kind) {
		case ID_VAR:
			...
			// check void_type_declaration
			if(isVoidType(id->type))
				semantic_error(100, id->line, "");
			// check empty array
...
BOOLEAN isCompatiblePointerType(A_TYPE *t1, A_TYPE *t2) {
	if (isPointerOrArrayType_sem(t1) && isPointerOrArrayType_sem(t2)) 
		return(isCompatibleType(t1->element_type,t2->element_type));
	else
		return(FALSE);
}
...
BOOLEAN isAllowableAssignmentConversion(A_TYPE *t1, A_TYPE *t2, A_NODE *node) // t1 <--- t2
{
	if (isArithmeticType(t1) && isArithmeticType(t2)) 
		...
		return (TRUE);
	else if (isPointerType(t1) && !isCompatiblePointerType(t1, t2)){
		semantic_warning(11, node->line);
		return (TRUE);
	}
	else if (isPointerType(t1) && isArithmeticType(t2)){
		semantic_warning(11, node->line);
		return (TRUE);
	}
	else if (isArithmeticType(t1) && isPointerOrArrayType_sem(t2)){
		semantic_warning(11, node->line);
		return (TRUE);
	}
	else
...
A_LITERAL getTypeAndValueOfExpression(A_NODE *node) {
	...
	switch(node->name) {
		...
		case N_EXP_ADD : //여기 때문에 제대로 더하고 빼는값에 문제가 생기나보다
			result=getTypeAndValueOfExpression(node->llink); 
			r=getTypeAndValueOfExpression(node->rlink);
			if (result.type==int_type && r.type==int_type){
				result.type=int_type;
				...
				semantic_error(18,node->line,""); 
			break;
		case N_EXP_SUB : //int , float간의 연산만 지원하잖아 여기서는
			result=getTypeAndValueOfExpression(node->llink); 
			r=getTypeAndValueOfExpression(node->rlink);
			if (result.type==int_type && r.type==int_type){
				result.type=int_type;
...
void semantic_error(int i, int ll, char *s) {
	...
	//error in expression
		case 13: printf("arith type expr required in unary operation\n");
			break;
		case 18: printf("illegal constant exprresion\n");
			break;
		...
		case 31: printf("pointer type expr required in pointer operation\n");
			break;
		case 32: printf("array type expr required in array expression\n");
			break;
		case 34: printf("too many arguments in function call\n");
			break;
		case 35: printf("too few arguments in function call\n");
			break;
		case 37: printf("illegal struct field indentifier in reference expr\n");
			break;
		...
		case 60: printf("expression is not an lvalue\n");
			break;
		case 71: printf("case lable not within a switch statement\n");
			break;
		case 72: printf("default label not with in a switch statement\n");
			break;
	...
	//misc errors
		case 90: printf("fatal compiler error in parse result\n");
			break;
		case 93: printf("too many literals in sourse program\n");
			break;	
	//user custom:
		case 100: printf("void type specifier in variable or paramter\n");
			break;
...
void semantic_warning(int i, int ll)
{
	...
	switch (i){
		...
		case 12:printf("incompatible types in argument or return expr\n");
			break;
		case 14:printf("incompatible types in bynary expression\n");
			break;
