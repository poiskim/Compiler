...
A_ID *filterVoidParameter(A_ID *id){
	A_ID *m = id;
	int cnt = 0, sum = 0; //void type의 개수 세기
	...
	while(m){
		...
		if(m->type == void_type) {
			...
			//identifier가 없는지 확인하고 있다면 에러후 NIL리턴
			if(strcmp(m->name, "") != 0){
				syntax_error(14, "");
				return NIL;
			}
	...
	//void type이 하나도 없다면 평소처럼 진행
	if(cnt <= 0) return id;
	else{
		if(sum == 1 && cnt == 1); //유일한 파라미터가 void형이었다면
		else syntax_error(14, "");
...
A_ID *setParameterDeclaratorSpecifier (A_ID *id, A_SPECIFIER *p) { 
	...
	if (p -> stor)	//void가 여기서 걸려 죽음
		syntax_error(14, "");
	else if(p -> type == void_type){
		//일단 보이드 타입이라면 만들고 나중에 생각함
	}
	setDefaultSpecifier(p);
	id = setDeclaratorElementType(id, p -> type);
	id -> kind = ID_PARM;
...
BOOLEAN isPointerOrArrayType(A_TYPE *t) {
	//t가 있고 t의 종류가 pointer 혹은 array라면 true 아니라면 false
	if (t && ( t->kind == T_POINTER || t->kind == T_ARRAY)) 
		return(TRUE);
	return(FALSE);
}

void checkArgList(A_NODE *n_name, A_NODE *n_list){
	A_ID *id = (A_ID *)(n_name->clink), *f = NIL;
	A_NODE *n = n_list;
	if(!id) return;
	f = id->type->field;
	if(id->type->kind != T_FUNC) {
		syntax_error(101, id->name);
		return;
	}
}

A_NODE *getNodeOfIdentifierOfStructOrUnion(A_NODE *n, char *s, NODE_NAME n_name){
	A_ID *id;
	A_NODE *m;
	A_TYPE *t;
	...
	if(n->rlink) n = n->rlink;

	//clink가 있고 이 함수가 불리면 iD)
	if(n->clink){
		t = ((A_ID *)n->clink)->type;

		if(n_name == N_EXP_ARROW){
			if(t->element_type->kind != T_STRUCT && t->element_type->kind != T_UNION){
				syntax_error(11, "");
				return NIL;
			}
			else id = t->element_type->field;
		}
		else if(n_name == N_EXP_STRUCT){
		...
		while(id){
			if(strcmp(s, id->name) == 0){
				return makeNode(N_EXP_IDENT, NIL, id, NIL);
			}
			id = id->link;
		}
...
void syntax_error(int i,char *s) {
	...
	switch (i) { 
		...
		case 101:
			printf("%s is not a function or function pointer", s);
			break;
